...
class NearestNeighborSearcher:
	...
	def processDocsInBatches(self, documents):
		...
		totalBatches = (len(documents) / self.batchSize ) + 1    
		...
		batchEnd = min(((batchNo+1) * self.batchSize), len(documents))
		...
		self.annoy_index.build(self.batchSize) 
	...
	def processTokensToEmbeddings(self, documents):
		...
		for batchNo in range(0, totalBatches):
			...
			try:
				...
				print(f"Failed Processing batch {batchNo+1} of {int(totalBatches)}. Range {batchStart} to {batchEnd} ||   " + datetime.datetime.now().isoformat())
				print(e)
...
if __name__ == "__main__":
	...
	nns = NearestNeighborSearcher(batchSize=1000, index_name="src/main/resources/arxiv_transformer_index.bin")
